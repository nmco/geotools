...
public class JsonSelectFunction extends FunctionExpressionImpl {
    ...
    public Object evaluate(Object object) {
        ...
        if (this.params.get(0) != null) {
            selectAll = Boolean.getBoolean(this.params.get(0).toString());
        }
        if(mongoObjectHolder.getCollectionInfoHolder().containsCollectionInfo()) {
            Map<String, String> copyCollectionsIds = new HashMap<>(mongoObjectHolder.getCollectionInfoHolder().getObjectsIds());
            copyCollectionsIds.put(mongoObjectHolder.getCollectionInfoHolder().getCollectionPath(),
                    String.valueOf(mongoObjectHolder.getSubCollectionIndex()));
            return MongoComplexUtilities.jsonSelect(mongoObjectHolder.getObject(),
                    copyCollectionsIds, this.params.get(0).toString(), selectAll);
        }
        return MongoComplexUtilities.jsonSelect(mongoObjectHolder.getObject(),
                mongoObjectHolder.getCollectionInfoHolder().getObjectsIds(),
                this.params.get(0).toString(), selectAll);
